...
## JSONPlaceholder API에서 데이터 가져오기 — 계획서

목표: 개발 중 실제 서버 없이 외부 API를 사용해 목록/상세 데이터를 받아와서 UI와 데이터 연동 흐름을 검증한다.

요구사항 요약:
- 안전한 fetch 패턴(서버 컴포넌트에서 SSR fetch + 캐싱 옵션)
- 클라이언트에서만 필요한 경우 `useEffect` 기반 페칭
- 에러/로딩 상태와 재시도 전략 명시
- 타입 정의 (`Post` 타입)와 변환(필요시)

단계별 계획:
1. 엔드포인트 선정
   - 목록: `https://jsonplaceholder.typicode.com/posts?_limit=10`
   - 상세: `https://jsonplaceholder.typicode.com/posts/{id}`

2. 서버 컴포넌트에서 초기 목록 가져오기
   - 위치: `app/posts/page.tsx` (Server Component)
   - 코드: `const res = await fetch(url, { next: { revalidate: 60 } })` — 60초 캐시
   - 장점: SEO, 초기 렌더링 빠름, API 키 노출 위험 없음

3. 클라이언트에서 추가 인터랙션용 페칭
   - 검색 결과나 페이징 같이 사용자 입력으로 자주 호출하면 `useEffect`로 클라이언트 페칭
   - 로컬 상태(`useState`)로 로딩/에러 관리

4. 타입 및 에러 처리
   - `type Post = { id: number; title: string; body: string; userId?: number }`
   - `try/catch`로 네트워크 에러 처리, HTTP 상태 코드 검사
   - 실패 시 사용자 메시지와 재시도 버튼 제공

5. 점진적 개선
   - 서버에서 받아온 데이터를 클라이언트로 전달하고 클라이언트 내에서 필터/삭제(로컬) 기능을 구현
   - 이후 실제 백엔드(Supabase 등)로 교체 시 API 호출 위치만 교체

예시 코드 스니펫 (서버 컴포넌트에서 목록 가져오기):
...
export default async function PostsPage() {
  ...
  return <PostsClient initialPosts={posts} />; // PostsClient는 "use client"
}
```

예시 코드 스니펫 (클라이언트 컴포넌트에서 useEffect):
...
테스트 체크리스트:
- 네트워크 차단 시 에러 UI 확인
- 캐시 옵션(`revalidate`) 변경 시 데이터 갱신 동작 확인
- 클라이언트 검색/삭제가 로컬 상태만 변경하는지 확인(서버 호출 없음)

참고: JSONPlaceholder는 학습/테스트용 공개 API입니다. 실제 서비스에서는 CORS, 인증, 속도 제한, 데이터 형식 등을 고려해야 합니다.
```
