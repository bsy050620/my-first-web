...
## Ch11 전용 규칙 (RLS & 데이터 보호)

- **RLS 적용 방식**: Supabase CLI 마이그레이션 파일(`supabase/migrations/`)을 통해 정책을 작성합니다. SQL Editor에서 직접 실행하지 않습니다.
- **정책 기준**: `posts` 테이블에 대해 `user_id`와 `auth.uid()`를 기준으로 정책을 생성합니다.
  - `SELECT`: 모든 사용자가 공개 포스트 조회 가능
  - `INSERT`: 인증된 사용자만 삽입 가능
  - `UPDATE`: 작성자(`user_id` = `auth.uid()`)만 수정 가능
  - `DELETE`: 작성자만 삭제 가능
- **클라이언트 UI는 보안이 아님**: 수정/삭제 버튼을 숨기는 것은 UX일 뿐, 실제 데이터 보호는 RLS 정책이 담당합니다.
...
- **서버 API 검증** (선택): 마이그레이션 후 필요시 서버 API에서 추가 검증(예: 사용자 ID 확인)을 구현할 수 있습니다.
