...
- **RLS 마이그레이션**: 정책은 `supabase/migrations/` 디렉토리에 SQL 파일로 작성하고, SQL Editor에서 직접 실행하지 않습니다.
- **정책 기준**: `posts` 테이블에서 `user_id` = `auth.uid()` 기준으로 UPDATE/DELETE를 제어합니다.
- **클라이언트 UI 분기는 UX일 뿐**: 수정/삭제 버튼 노출 여부는 UX 레벨이며, 실제 보안은 RLS 정책이 담당합니다.
- **service_role 키 금지**: 클라이언트 코드(`lib/supabase/client.ts`, `contexts/AuthContext.tsx`, `app/` 페이지)에서 service_role 키를 사용하면 안 됩니다.
